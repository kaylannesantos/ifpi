...
----------------------------------------------------FUNÇÃO CREDIÁRIO---------------------------------------------------------------
CREATE OR REPLACE FUNCTION CARNE_CREDIARIO(CPF_C VARCHAR(50), QUANT_P INT, DT_V DATE)
RETURNS VOID AS $$
DECLARE
	COD_C INT;
	COD_P INT;
...
BEGIN
	SELECT COD INTO COD_C FROM CLIENTE WHERE CPF ILIKE CPF_C;
	SELECT COD INTO COD_P FROM PEDIDO P JOIN CLIENTE C ON C.COD = P.COD_CLIENTE WHERE CPF ILIKE CPF_C;
	
	--VERIFICACAO DE DATA DE VENCIMENTO COM A DATA DO PAGAMENTO
	
	IF NOT EXISTS(SELECT * FROM CLIENTE C JOIN PEDIDO P ON P.COD_CLIENTE = C.COD WHERE C.COD = COD_C AND P.COD = COD_P) THEN
		IF QUANT_P > 0 AND QUANT_P <= 6 THEN --SE O N° FOR MAIOR QUE 0 E MENOR OU IGUAL A 6 = SEM JUROS
			VALOR_P := (VALOR_TOTAL / QUANT_P) FROM PEDIDO; --VALOR TOTAL DO PEDIDO DIVIDO PELA QUANTIDADE DE PARCELAS
	
			INSERT INTO CREDIARIO(COD_PEDIDO,QUANTIDADE_PARCELA,VALOR_PARCELA,DATA_VENCIMENTO) VALUES(COD_P, QUANT_P, VALOR_P, DT_V);
	
		ELSIF QUANT_P > 6 AND QUANT_P <= 12 THEN --SE O N° DE PARCELAS FOR MAIOR QUE 6 E MENOR OU IGUAL A 12 = 0.5% DE JUROS
			VALOR_J := VALOR_TOTAL * 0.05 FROM PEDIDO; --VALOR DO JUROS EM CIMA DO VALOR TOTAL
			VALOR_P := (VALOR_TOTAL + VALOR_J) / QUANT_P FROM PEDIDO; --VALOR TOTAL SOMADO AO VALOR DO JUROS DIVIDO PELA QUANTIDADE DE PARCELAS
	
			INSERT INTO CREDIARIO(COD_PEDIDO,QUANTIDADE_PARCELA,VALOR_PARCELA,DATA_VENCIMENTO) VALUES(COD_P, QUANT_P, VALOR_P, DT_V);
		ELSE
			RAISE EXCEPTION 'A QUANTIDADE DE PARCELAS % EXCEDE A QUANTIDADE PERMITIDA', QUANT_P;
		END IF;	
	ELSE
		RAISE EXCEPTION 'O CLIENTE NÃO POSSUI CARNÊ A PAGAR.';
	END IF;
	
END;
$$ LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION PAGAR_CARNE()
RETURNS VOID AS $$
DECLARE
