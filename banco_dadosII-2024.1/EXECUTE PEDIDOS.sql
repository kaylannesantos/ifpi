					...
					IF (QUANT_E >= QUANT_V) THEN
						...
						IF EXISTS(SELECT * FROM PAGAMENTO WHERE COD = COD_PAG) THEN
							...
							IF EXISTS(SELECT * FROM PEDIDO WHERE COD = COD_PDD) THEN
								...
								ELSIF EXISTS(SELECT * FROM ITEM_PEDIDO IP JOIN PEDIDO PDD ON PDD.COD = IP.COD_PEDIDO JOIN ESTOQUE E ON E.COD = IP.COD_ESTOQUE JOIN PRODUTO P ON P.COD = E.COD_PRODUTO JOIN LOJA L ON L.COD = E.COD_LOJA WHERE IP.COD_PEDIDO = COD_PDD AND E.COD_PRODUTO = COD_P AND IP.COD_ESTOQUE = E.COD AND E.COD_LOJA = COD_L) THEN
									--ITEM PEDIDO
...
DECLARE
	...
	PROX_COD INT;
BEGIN
	SELECT C.COD, C.VALOR_PARCELA, C.DATA_PAGAMENTO, C.DATA_VENCIMENTO INTO COD_C, VALOR_P, DATA_P, DATA_V
	FROM CREDIARIO C JOIN PEDIDO P ON P.COD = C.COD_PEDIDO JOIN CLIENTE CLI ON CLI.COD = P.COD_CLIENTE
	WHERE CLI.CPF ILIKE CPF_C AND C.DATA_PAGAMENTO IS NULL ORDER BY C.DATA_VENCIMENTO LIMIT 1;
	
	SELECT EXTRACT(DAY FROM DATA_PAG - DATA_V) INTO DIAS_ATRASO FROM CREDIARIO;
	
	IF DATA_PAG IS NOT NULL THEN
...
SELECT ADD_PEDIDO('789.123.456-78', 'Sapato Anabela Feminino', 3, 'Crediário','901.777.567-99',3)
	
SELECT PAGAR_CREDIARIO('789.123.456-78', '30-09-2024')
...
SELECT C.COD INTO PROX_COD FROM CREDIARIO C 
JOIN PEDIDO P ON P.COD = C.COD_PEDIDO 
JOIN CLIENTE CLI ON CLI.COD = P.COD_CLIENTE
WHERE CLI.CPF ILIKE CPF_C AND C.COD_PEDIDO = (SELECT COD_PEDIDO FROM CREDIARIO WHERE COD = COD_C) 
AND C.COD > COD_C AND C.DATA_PAGAMENTO IS NULL ORDER BY C.DATA_VENCIMENTO LIMIT 1;	