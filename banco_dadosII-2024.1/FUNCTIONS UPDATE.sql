...
BEGIN
	...
	RETURN QUERY
	SELECT cod, nome, cpf, contato, email FROM CLIENTE WHERE (CPF ILIKE CPF_C OR CPF ILIKE IDENTIFICADOR_C) OR (NOME ILIKE NOME_C OR NOME ILIKE IDENTIFICADOR_C);
END;
$$ LANGUAGE PLPGSQL;
...
BEGIN 
	...
	RETURN QUERY 
	SELECT cod, nome, salario, comissao FROM CARGO WHERE NOME ILIKE COALESCE(NOME_CAR, IDENTIFICADOR_NOME);
END;
$$ LANGUAGE PLPGSQL;
...
BEGIN	
	...
	RETURN QUERY 
		...
		WHERE (CPF ILIKE CPF_F OR CPF ILIKE IDENTIFICADOR_F) OR (F.NOME ILIKE NOME_F OR F.NOME ILIKE IDENTIFICADOR_F);	
END;
$$ LANGUAGE PLPGSQL;
...
$$ LANGUAGE PLPGSQL;
--ex:
SELECT * FROM UPDATE_ESTOQUE('Vestido Longo de Festa-Loja 1', NULL, NULL,50) 
...
BEGIN 
	IF TABELA ILIKE 'CLIENTE' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_CLIENTE(IDENTIFICADOR,CPF,CLIENTE,CONTATO,EMAIL);
		ELSE
			RAISE EXCEPTION 'PARÂMETROS INCORRETOS.';
		END IF;
	ELSIF TABELA ILIKE 'CARGO' THEN
...
BEGIN 
	...
	ELSIF TABELA ILIKE 'FUNCIONARIO' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_FUNCIONARIO(IDENTIFICADOR, CPF, FUNCIONARIO, CONTATO, EMAIL, NOME_CAR, NOME_LOJ);
		ELSE 
			RAISE EXCEPTION 'PARÂMETROS INCORRETOS.';
		END IF;
	ELSIF TABELA ILIKE 'PAGAMENTO' THEN
...
$$ LANGUAGE PLPGSQL;
--SELECT * FROM CARGO
--SELECT * FROM ALTERAR('Cargo','Vendedor Lojista','Vendedor',NULL,NULL)
...
CREATE OR REPLACE FUNCTION ALTERAR(--PRODUTO DROP FUNCTION ALTERAR(VARCHAR,VARCHAR,VARCHAR,NUMERIC,VARCHAR)
	TABELA VARCHAR(50),
	IDENTIFICADOR VARCHAR(100),
	PRODUTO VARCHAR(100), 
	VALOR NUMERIC(8,2),
...
$$ LANGUAGE PLPGSQL;
--SELECT * FROM PRODUTO
--SELECT * FROM ALTERAR('PRODUTO', 'Cinto de Couro','Cinto de Couro, Nylon e Social',NULL,null)
...
SELECT * FROM LOJA
SELECT * FROM PRODUTO
SELECT * FROM ALTERAR('ESTOQUE', 'Calça Jeans Skinny-Loja 2', null, 'Loja 3', null)
