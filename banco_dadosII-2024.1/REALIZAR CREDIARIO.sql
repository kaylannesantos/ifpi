...
BEGIN
	...
	SELECT DATA_PAGAMENTO INTO DATA_P FROM CREDIARIO C WHERE QUANTIDADE_PARCELA = (SELECT MIN(QUANTIDADE_PARCELA) FROM CREDIARIO C WHERE C.COD_PEDIDO = COD_P AND DATA_PAGAMENTO IS NULL); --DATA PAGAMENTO
...
-------------------------------------------------------VER CARNÊ CREDIARIO-------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION VER_CARNE_CREDIARIO(COD_C INT)
RETURNS TABLE() AS $$
DECLARE
BEGIN
END;
...
SELECT ADD_PEDIDO('789.123.456-78', 'Vestido Longo de Festa', 5, 'Crediário','345.678.901-23',2)
	
SELECT * FROM FINALIZAR_PEDIDO('789.123.456-78')

SELECT PAGAR_CARNE('789.123.456-78', '04-09-2024')