----------------------------------------------------FUNÇÃO CREDIÁRIO---------------------------------------------------------------
CREATE OR REPLACE FUNCTION CARNE_CREDIARIO(COD_P INT, QUANT_P INT) --ADD_CARNE_CREDIARIO
RETURNS VOID AS $$
DECLARE
	COD_C INT; -- CREDIARIO
	TAXA_J NUMERIC(8,2);
...
BEGIN
	...
	SELECT C.COD, VALOR_PARCELA INTO COD_C, VALOR_P, DATA_V FROM CREDIARIO C JOIN PEDIDO P ON P.COD = C.COD_PEDIDO WHERE P.COD_CLIENTE = COD_CLI AND C.COD_PEDIDO = COD_P AND P.PAGO = TRUE AND DATA_PAGAMENTO IS NULL; --CREDIARIO
...
SELECT ADD_PEDIDO('987.654.321-00', 'Vestido Longo de Festa', 5, 'Crediário','345.678.901-23',4)
SELECT * FROM FINALIZAR_PEDIDO('456.789.123-45')
SELECT * FROM PAGAR_CARNE('456.789.123-45', '04-08-2024')
