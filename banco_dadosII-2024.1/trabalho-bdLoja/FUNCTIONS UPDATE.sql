...
BEGIN
    IF NEW.NOME IS NOT NULL THEN
        IF EXISTS (SELECT * FROM CLIENTE WHERE NOME = NEW.NOME AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O NOME % JÁ ESTÁ EM USO POR OUTRO CLIENTE.', NEW.NOME;
        END IF;
    END IF;
    
    IF NEW.CPF IS NOT NULL THEN
        IF EXISTS (SELECT * FROM CLIENTE WHERE CPF = NEW.CPF AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O CPF % JÁ ESTÁ EM USO POR OUTRO CLIENTE.', NEW.CPF;
        END IF;
    END IF;
    
    IF NEW.EMAIL IS NOT NULL THEN
        IF EXISTS (SELECT * FROM CLIENTE WHERE EMAIL = NEW.EMAIL AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O EMAIL % JÁ ESTÁ EM USO POR OUTRO CLIENTE.', NEW.EMAIL;
        END IF;
    END IF;

    IF NEW.CONTATO IS NOT NULL THEN
        IF EXISTS (SELECT * FROM CLIENTE WHERE CONTATO = NEW.CONTATO AND COD <> NEW.COD) THEN
...
BEGIN
    ...
    IF NEW.ENDERECO IS NOT NULL THEN
        IF EXISTS (SELECT * FROM LOJA WHERE ENDERECO = NEW.ENDERECO AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O ENDERECO % JÁ ESTÁ EM USO POR OUTRO LOJA.', NEW.ENDERECO;
        END IF;
    END IF;
...
BEGIN
    IF NEW.NOME IS NOT NULL THEN
        IF EXISTS (SELECT * FROM FUNCIONARIO WHERE NOME = NEW.NOME AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O NOME % JÁ ESTÁ EM USO POR OUTRO FUNCIONÁRIO.', NEW.NOME;
        END IF;
    END IF;

    IF NEW.CPF IS NOT NULL THEN
        IF EXISTS (SELECT * FROM FUNCIONARIO WHERE CPF = NEW.CPF AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O CPF % JÁ ESTÁ EM USO POR OUTRO FUNCIONÁRIO.', NEW.CPF;
        END IF;
    END IF;

    IF NEW.CONTATO IS NOT NULL THEN
        IF EXISTS (SELECT * FROM FUNCIONARIO WHERE CONTATO = NEW.CONTATO AND COD <> NEW.COD) THEN
            RAISE EXCEPTION 'O CONTATO % JÁ ESTÁ EM USO POR OUTRO FUNCIONÁRIO.', NEW.CONTATO;
        END IF;
    END IF;
	
    IF NEW.EMAIL IS NOT NULL THEN
        IF EXISTS (SELECT * FROM FUNCIONARIO WHERE EMAIL = NEW.EMAIL AND COD <> NEW.COD) THEN
...
BEGIN
    IF NEW.NOME IS NOT NULL THEN
        IF EXISTS (SELECT * FROM PAGAMENTO WHERE NOME = NEW.NOME AND COD <> NEW.COD) THEN
...
BEGIN
    IF NEW.NOME IS NOT NULL THEN
        IF EXISTS (SELECT * FROM CATEGORIA WHERE NOME = NEW.NOME AND COD <> NEW.COD) THEN
