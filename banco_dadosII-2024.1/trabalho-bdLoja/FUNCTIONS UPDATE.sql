...
BEGIN
	...
	RETURN QUERY 
		...
		WHERE 
			(E.COD_PRODUTO = COD_PROD AND E.COD_LOJA = COD_L) OR 
			(E.COD_PRODUTO = COD_P AND E.COD_LOJA = COD_LOJ) OR 
			E.COD_PRODUTO = E.COD_LOJA;
END;
$$ LANGUAGE PLPGSQL;
...
BEGIN 
	...
	END IF;
END;
$$ LANGUAGE PLPGSQL;--erro de reconhecimento no tipo de atribut
...
CREATE OR REPLACE FUNCTION ALTERAR(--LOJA DROP FUNCTION ALTERAR(VARCHAR,VARCHAR,VARCHAR,varCHAR,text)
	TABELA VARCHAR(50),
	IDENTIFICADOR VARCHAR(50),
	LOJA_PAGAMENTO VARCHAR(50), --LOJA VARCHAR(20) DEFAULT NULL,
	CNPJ VARCHAR(18),
	ENDERECO TEXT)
RETURNS TABLE(
	"id°" int,
...
BEGIN
	...
	ELSIF TABELA ILIKE 'PAGAMENTO' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_PAGAMENTO(IDENTIFICADOR, LOJA_PAGAMENTO);
		ELSE
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSIF TABELA ILIKE 'FUNCIONARIO' THEN
	...
	ELSIF TABELA ILIKE 'LOJA' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY 
			SELECT * FROM UPDATE_LOJA(IDENTIFICADOR, LOJA_PAGAMENTO, CNPJ, ENDERECO);
		ELSE 
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE .';
		END IF;
	ELSIF TABELA ILIKE 'CATEGORIA' THEN
...
CREATE OR REPLACE FUNCTION ALTERAR(--FUNCIONARIO DROP FUNCTION ALTERAR(VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR)
	...
	CLIENTE_FUNCIONARIO VARCHAR(100) DEFAULT NULL, --FUNCIONARIO VARCHAR(100) DEFAULT NULL,
...
BEGIN 
	IF TABELA ILIKE 'CLIENTE' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_CLIENTE(IDENTIFICADOR,CLIENTE_FUNCIONARIO,CPF,CONTATO,EMAIL);
		ELSE
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSIF TABELA ILIKE 'CARGO' THEN
	...
	ELSIF TABELA ILIKE 'FUNCIONARIO' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_FUNCIONARIO(IDENTIFICADOR,CLIENTE_FUNCIONARIO,CPF,CONTATO,EMAIL,NOME_CAR,NOME_LOJ);
		ELSE 
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSIF TABELA ILIKE 'PAGAMENTO' THEN
...
CREATE OR REPLACE FUNCTION ALTERAR(--ESTOQUE DROP FUNCTION ALTERAR(varchar,varchar,VARCHAR,VARCHAR,INT)
	...
	IDENTIFICADOR VARCHAR(100),
	PRODUTO VARCHAR(50), 
	CATEGORIA_LOJA VARCHAR(50), --LOJA VARCHAR(50) DEFAULT NULL
	VALOR_QUANTIDADE NUMERIC)--QUANTIDADE NUMERIC DEFAULT NULL
RETURNS TABLE(
	"id°" int,
	"Produto" varchar,
	"Loja" varchar,
...
BEGIN 
	...
	ELSIF TABELA ILIKE 'PRODUTO' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			PERFORM UPDATE_PRODUTO(IDENTIFICADOR, PRODUTO, CATEGORIA_LOJA, VALOR_QUANTIDADE);
		ELSE
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSIF TABELA ILIKE 'ESTOQUE' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_ESTOQUE(IDENTIFICADOR,PRODUTO,CATEGORIA_LOJA,VALOR_QUANTIDADE::INT);
		ELSE 
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSE
	...
	END IF;
END;
$$ LANGUAGE PLPGSQL;--erro de reconhecimento no tipo de atributo e retorno da query
SELECT * FROM estoque
SELECT * FROM produto
SELECT * FROM ALTERAR('produto','Calça Jeans Reta','Calça Jeans Wide Leg',null,null::int)
...
BEGIN
	...
	END IF;	
END;
$$ LANGUAGE PLPGSQL;--da erro de 'nao é unica'
...
CREATE OR REPLACE FUNCTION ALTERAR(--PRODUTO DROP FUNCTION ALTERAR(varchar,VARCHAR,VARCHAR,VARCHAR,float)
	TABELA VARCHAR(7),
	IDENTIFICADOR VARCHAR(100),
