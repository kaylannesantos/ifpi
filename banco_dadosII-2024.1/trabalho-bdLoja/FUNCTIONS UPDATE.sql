...
BEGIN	
	...
	ELSE
		...
		IF NOME_LOJ IS NOT NULL THEN
			...
			ELSE
				RAISE EXCEPTION 'A % NÃO EXISTE.', NOME_LOJ;
			END IF;
...
select * from update_funcionario('Carlos Santos', 'João da Silva', null,null, 'jo.silva@email.com',null,null)
...
BEGIN
	...
	ELSE 
		...
		IF NOT FOUND THEN
			RAISE EXCEPTION 'FALHA NA ATUALIZAÇÃO DO PRODUTO % e %', NOME_P, NOME_L;
		ELSE
...
BEGIN
	...
	ELSIF TABELA ILIKE 'LOJA' THEN
		IF IDENTIFICADOR IS NOT NULL THEN
			RETURN QUERY 
			SELECT * FROM UPDATE_LOJA(IDENTIFICADOR, LOJA, CNPJ, ENDERECO::VARCHAR);
		ELSE 
...
BEGIN 
	...
	END IF;
END;
$$ LANGUAGE PLPGSQL; --erro nao e tabela unica
...
/*--------------------------------------------------2°TENTATIVA-----------------------------------------------------------
CREATE OR REPLACE FUNCTION UPDATE_ESTOQUE( --DROP FUNCTION UPDATE_ESTOQUE2
...
BEGIN
	...
	ELSE 
		...
		IF NOT FOUND THEN
			RAISE EXCEPTION 'FALHA NA ATUALIZAÇÃO DO PRODUTO % E %', IDENTIFICADOR_P, IDENTIFICADOR_L;
		ELSE
			RAISE NOTICE 'ESTOQUE ATUALIZADO COM SUCESSO.';
		END IF;
	END IF;
...
BEGIN 
	...
	ELSIF TABELA ILIKE 'ESTOQUE' THEN
		IF IDENTIFICADOR_P IS NOT NULL AND IDENTIFICADOR_L IS NOT NULL THEN
			RETURN QUERY
			SELECT * FROM UPDATE_ESTOQUE(IDENTIFICADOR_P, IDENTIFICADOR_L,PRODUTO,LOJA,QUANTIDADE);
		ELSE 
			RAISE EXCEPTION 'PARÂMETRO IDENTIFICADOR VÁZIO. POR FAVOR, TENTE NOVAMENTE.';
		END IF;
	ELSE
