...
SELECT 
	...
	C.NOME as "Cargo",
	L.NOME as "Loja",
	'R$ ' || TO_CHAR(C.SALARIO, '999G999D00') as "Salário Bruto",
	CASE
		WHEN COALESCE(SUM(CF.COMISSAO), 0) = 0 THEN 'Sem Comissão'
		ELSE 'R$ ' || TO_CHAR(COALESCE(SUM(CF.COMISSAO), 0), '999G999D00')
	END as "Comissão",
	'R$ ' || TO_CHAR((C.SALARIO + COALESCE(SUM(CF.COMISSAO), 0)),'999G999D00') as "Salario Líquido"
FROM FUNCIONARIO F
...
BEGIN
	...
	RETURN QUERY
		SELECT 
			...
			C.NOME,
			L.NOME,
			'R$ ' || TO_CHAR(C.SALARIO, '999G999D00'),
			CASE
				WHEN COALESCE(SUM(CF.COMISSAO), 0) = 0 THEN 'Sem Comissão'
				ELSE 'R$ ' || TO_CHAR(COALESCE(SUM(CF.COMISSAO), 0), '999G999D00')
			END as comissao,
			'R$ ' || TO_CHAR((C.SALARIO + COALESCE(SUM(CF.COMISSAO), 0)),'999G999D00')
		FROM FUNCIONARIO F
			JOIN CARGO C ON C.COD = F.COD_CARGO
			JOIN LOJA L ON L.COD = F.COD_LOJA
			LEFT JOIN COMISSAO_FUNCIONARIO CF ON CF.COD_FUNCIONARIO = F.COD
