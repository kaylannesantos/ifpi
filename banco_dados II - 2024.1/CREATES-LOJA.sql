...
CREATE OR REPLACE FUNCTION ADD_FUNCIONARIO(COD_LF, NOME_F, CPF_F, CONTATO_F, EMAIL_F)
RETURNS VOID AS $$
DECLARE 
	COD_C INT;
BEGIN
	IF NOT EXISTS(SELECT * FROM FUNCIONARIO WHERE CPF = CPF_F) THEN 
		IF EXISTS(SELECT * FROM CARGO WHERE COD_CARGO = COD_C) THEN
		ELSE
			RAISE EXCEPTION 'O CARGO JÁ EXISTE.'
	ELSE
		'O FUNCIONARIO JÁ EXISTE.';
	END IF;
END;
$$
LANGUAGE PLPGSQL;