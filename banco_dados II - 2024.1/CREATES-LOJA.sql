...
BEGIN	
	...
	ELSE 
		IF NOME_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET NOME = NOME_F
			WHERE CPF = CPF_F;
		END IF;

		IF CONTATO_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET CONTATO = CONTATO_F
			WHERE CPF = CPF_F;
		END IF;

		IF EMAIL_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET EMAIL = EMAIL_F
			WHERE CPF = CPF_F;
		END IF;
		...
		IF NOME_CAR IS NOT NULL THEN
			...
			IF EXISTS(SELECT * FROM CARGO WHERE NOME ILIKE NOME_CAR) THEN
				UPDATE FUNCIONARIO SET COD_CARGO = COD_CAR
...
$$ LANGUAGE PLPGSQL;
--ex:
SELECT UPDATE_FUNCIONARIO('123.456.789-01', NULL, 'Loja 1', NULL, NULL, NULL)*/
...
$$ LANGUAGE PLPGSQL;
--ex:
SELECT UPDATE_PRODUTO('Sapatos','Cinto de Couro',null)
...
BEGIN
	IF NOME_PROD IS NOT NULL THEN 
		...
		END IF;
	ELSIF NOME_LOJ IS NOT NULL THEN 
		SELECT COD_LOJA INTO COD_LOJ FROM LOJA WHERE NOME ILIKE NOME_P;

		IF EXISTS(SELECT * FROM LOJA WHERE NOME ILIKE NOME_LOJA) THEN 
			RAISE EXCEPTION 'A LOJA NÃO EXISTE.';
		ELSE
