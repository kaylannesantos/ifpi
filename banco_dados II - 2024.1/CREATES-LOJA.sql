	...
	ELSE
		IF NOME_CAR IS NOT NULL THEN
			...
			IF EXISTS(SELECT * FROM CARGO WHERE COD_CARGO = COD_CAR) THEN
				UPDATE FUNCIONARIO SET COD_CARGO = COD_CAR WHERE COD_FUNCIONARIO = COD_F;
			ELSE
		...
		IF NOME_LOJ IS NOT NULL THEN
			...
			IF EXISTS(SELECT * FROM LOJA WHERE COD_LOJA = COD_LOJ) THEN
				UPDATE FUNCIONARIO SET COD_LOJA = COD_LOJ WHERE COD_FUNCIONARIO = COD_F;
			ELSE
		...
		IF NOME_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET NOME = NOME_F WHERE COD_FUNCIONARIO = COD_F;
		END IF;
	
		IF CPF_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET CPF = CPF_F WHERE COD_FUNCIONARIO = COD_F;
		END IF;
	
		IF CONTATO_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET CONTATO = CONTATO_F WHERE COD_FUNCIONARIO = COD_F;
		END IF;
	
		IF EMAIL_F IS NOT NULL THEN
			UPDATE FUNCIONARIO SET EMAIL = EMAIL_F WHERE COD_FUNCIONARIO = COD_F;
		END IF;
...
RETURNS VOID AS $$ 
BEGIN 
	IF NOT EXISTS(SELECT * FROM CATEGORIA WHERE COD_CATEGORIA = COD_C) THEN
		RAISE EXCEPTION 'A CATEGORIA NÃO EXISTE.';
	ELSE
		UPDATE CATEGORIA SET 
			NOME = COALESCE(NOME_C, NOME),
			DESCRICAO = COALESCE(DESC_C, DESCRICAO)
		WHERE COD_CATEGORIA = COD_C;
	END IF;
END;
$$ LANGUAGE PLPGSQL;
--
...
DECLARE
	COD_CAT INT;
BEGIN 
	IF NOT EXISTS(SELECT * FROM PRODUTO WHERE COD_PRODUTO = COD_P) THEN
		RAISE EXCEPTION 'O PRODUTO % NÃO EXISTE.', NOME_P;
	ELSE 
		IF NOME_CAT IS NOT NULL THEN
			SELECT COD_CATEGORIA INTO COD_CAT FROM CATEGORIA WHERE NOME ILIKE NOME_CAT;
...
$$ LANGUAGE PLPGSQL;
--
SELECT UPDATE_PRODUTO(1,'Vestido longos',NULL,NULL )
...
RETURNS VOID AS $$ 
BEGIN 
	IF NOT EXISTS(SELECT * FROM PAGAMENTO WHERE COD_PAGAMENTO = COD_P) THEN
		RAISE EXCEPTION 'O TIPO DE PAGAMENTO NÃO EXISTE.';
	ELSE 
		UPDATE PAGAMENTO SET TIPO = COD_PAGAMENTO = COD_P;
	END IF;
END;
$$ LANGUAGE PLPGSQL;
--ex:
...
DECLARE
	...
	COD_LOJ INT;
BEGIN
	IF NOT EXISTS(SELECT * FROM ESTOQUE WHERE COD_ESTOQUE = COD_E) THEN 
		RAISE EXCEPTION 'O PRODUTO NÃO EXISTE.';
	ELSE 
		IF NOME_PROD IS NOT NULL THEN
			...
			ELSE 
				UPDATE ESTOQUE SET COD_PRODUTO = COD_PROD WHERE COD_ESTOQUE = COD_E;
			END IF;
		END IF;
		...
		IF NOME_LOJ IS NOT NULL THEN
			...
			IF NOT EXISTS(SELECT * FROM LOJA WHERE NOME ILIKE NOME_LOJ) THEN 
				RAISE EXCEPTION 'A LOJA % NÃO EXISTE.', NOME_LOJ;
			ELSE
				UPDATE ESTOQUE SET COD_LOJA = COD_LOJ WHERE COD_ESTOQUE = COD_E;
			END IF;
		END IF;
